...
Drei der fünf Aktionen zeigen Symbol und Beschriftung, zwei zeichnen
selbst: Zuschauer und Streamzustand. Ein Blatt, das für diese beiden ein
...
**Auf diesen Bildern steht keine einzige Zahl von YouTube.** Ohne
Anmeldung gibt es keine Zuschauer, und ausgedacht wird hier nichts.
...
def _plugin():
    ...
    spec = importlib.util.spec_from_file_location(
        "youtube_bilder", ORDNER / "plugin.py",
...
async def _erzeuge(ziel: Path, werkzeug) -> list[str]:
    ...
    def ctx(aktion: dict, art="key"):
        ...
        slot = Slot(plugin_id="youtube", action_id=aktion["id"], settings=werte,
                    appearance=aussehen)
        return SlotContext(
    ...
    zeichner = ImageDraw.Draw(blatt)
    schrift = werkzeug._schrift(15)
    text = "Auf den Dials — hier ohne Verbindung zu YouTube"
    kasten = zeichner.textbbox((0, 0), text, font=schrift)
