...
Vier der sechs Aktionen zeichnen ihre Kachel selbst: Sie tragen den
Zustand als farbigen Rand und die Helligkeit als Zahl. Ein Blatt, das
dafür ein Symbol malt, behauptete eine Darstellung, die es nicht gibt.
Also wird das Plugin geladen und jede Kachel wirklich von ihm gezeichnet.

**Auf diesen Bildern steht keine einzige Zahl von Govee.** Ohne
Schlüssel gibt es keine Lampen und keine Helligkeit, und ausgedacht wird
hier nichts. Die Kacheln zeigen deshalb, was auch eine frische
Installation zeigt. Alles andere — Namen, Symbole, Beschriftungen,
Farbvorgaben — kommt aus dem Manifest.
...
async def _erzeuge(ziel: Path, werkzeug) -> list[str]:
    ...
    def ctx(aktion: dict, art="key"):
        ...
        slot = Slot(plugin_id="govee", action_id=aktion["id"], settings=werte,
                    appearance=aussehen)
        return SlotContext(
    ...
    zeichner = ImageDraw.Draw(blatt)
    schrift = werkzeug._schrift(15)
    text = "Auf den Dials — hier ohne Verbindung zu Govee"
    kasten = zeichner.textbbox((0, 0), text, font=schrift)
